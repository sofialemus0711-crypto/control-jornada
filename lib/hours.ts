...
export function etiquetaMes(mesISO: string): string {
  ...
  const nombre = NOMBRES_MES[(mes ?? 1) - 1] ?? "";
  return `${nombre} ${anio}`;
}
...
/**
 * A partir de los resumenes semanales completos de un empleado (todas las
 * semanas con datos), arma el resumen mensual: toma las semanas cuyo lunes
 * cae dentro del mes solicitado, las numera en orden, y suma sus horas
 * extra. Una semana que cruza fin de mes queda asignada por completo al mes
 * de su lunes (asi el calculo de las 42h semanales sigue siendo exacto,
 * porque usa todos los dias reales de esa semana).
 */
export function calcularResumenMensual(
  resumenesSemanales: ResumenSemanal[],
  mesISO: string
): ResumenMensual {
  const semanasDelMes = resumenesSemanales
    .filter((r) => r.fechaInicio.slice(0, 7) === mesISO)
  ...
  const totalHoras =
    Math.round(semanasDelMes.reduce((s, w) => s + w.totalHoras, 0) * 100) /
    100;
  const totalHorasExtra =
    Math.round(semanasDelMes.reduce((s, w) => s + w.horasExtra, 0) * 100) /
    100;
