...
export default function AdminQRPage() {
  ...
  useEffect(() => {
    setCargando(true);
    fetch(`/api/qr?t=${cacheBuster}`)
      .then((res) => {
        const destino = res.headers.get("X-QR-Destino");
        setUrlLogin(destino ? decodeURIComponent(destino) : "");
      })
    ...
    <div className="space-y-6 animate-fade-in max-w-lg">
      ...
      <section className="bg-white rounded-2xl shadow-card p-8 flex flex-col items-center text-center">
        ...
        <p className="text-xs text-ink-500 mt-4 break-all font-mono-tabular">
          {cargando ? "Verificando direccion..." : urlLogin || "No se pudo confirmar la direccion codificada."}
        </p>
        <p className="text-[11px] text-ink-400 mt-1">
          Esta es la direccion exacta que contiene el codigo QR de arriba.
        </p>
